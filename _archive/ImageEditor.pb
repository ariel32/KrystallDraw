        ...
        Case #GADGET_Canvas 
          ...
          Select EventType() 
            ...
            Case #PB_EventType_LeftButtonUp 
              i+1 
              CreateImage(#IMAGE_ctrlz+i, 380, 380, 24) And StartDrawing(ImageOutput(#IMAGE_ctrlz+i)) 
              DrawImage(GetGadgetAttribute(#GADGET_Canvas, #PB_Canvas_Image), 0, 0) 
              StopDrawing() 
              ;Debug i 
            ...
            Case #PB_EventType_RightButtonUp ; Активируем отмену на один шаг 
               If i>1:i-1:EndIf 
              If StartDrawing(CanvasOutput(#GADGET_Canvas)) 
                         DrawImage(ImageID(#IMAGE_ctrlz+i), 0, 0) 
                StopDrawing() 
              EndIf 
             ;FreeImage(#IMAGE_ctrlz+i) ; Возможно, если не надо реализовать возврат 
